...
Let $D$ be a set of keys defined over an alphabet $\Sigma \cup \left\lbrace\text{EOS}\right\rbrace$, where $\text{EOS}$ (End of String) is a unique termination marker. The exact node count of the structure is the cardinality of all distinct prefixes generated by the keys:

$$ V_{\text{nodes}} = \left| \left\lbrace\text{prefix}(x,i) \mid x \in D, 0 \le i \le |x|\right\rbrace \right| $$
